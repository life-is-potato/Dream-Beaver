...
bool BarcodePrinter::configurePrinter(QString printerName)
{
    ...
    {
        mPrinter.setOutputFileName("C://Users//jasse//Desktop//barcode.pdf");
...
void BarcodePrinter::savebarcode(QString barcodeText) {
    ...
    // Create a QImage to render the content
    QImage barcodeImage(67.5 * MmToDot, 25.5 * MmToDot, QImage::Format_RGB32);
    barcodeImage.fill(Qt::white); // Fill with white background
    ...
    barcodeImage.save(&buffer, "PNG"); // Save barcode image as PNG format
    buffer.close();

    // Save the byte array to the database
    QSqlQuery q;
    q.prepare("UPDATE EQUIPEMENTS SET BARCODE = :barcode_pdf WHERE ID = :id");

    q.bindValue(":barcode_pdf", byteArray);
    q.bindValue(":id", barcodeText);

    if (!q.exec()) {
        qDebug() << "Error inserting PDF barcode into database:" << q.lastError().text();
    } else {
        qDebug() << "PDF barcode saved successfully!";
    }
}
