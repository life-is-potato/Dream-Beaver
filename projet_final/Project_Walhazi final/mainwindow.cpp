...
{
    QString batchFilePath = "kill.bat";
...
if (flag == 0 ){
    if (e->Add_element(id,name,type,qnt,disp,desc)) {
           QMessageBox::information(nullptr,"ADD"," Entity added ! ") ;
           ui->ID->clear();
           ui->name->clear();
           ui->desc->clear();
           ui->qnt->clear();
...
void MainWindow::on_modify_clicked()
{
    ...
    QString desc = ui->desc_2->toPlainText() ;
            ...
            if (e->Modify_element(id,name,type,qnt,disp,desc)) {
                   QMessageBox::information(nullptr,"Modify","Entity Modifyed ! ") ;
                   ui->tableView->setModel(e->afficher());
                   chart_render() ;
                   ui->equipments_widget->setCurrentIndex(0);
            }
            else {
                   QMessageBox::critical(nullptr,"Failed"," Something went wrong ! ") ;
            }
...
void MainWindow::on_delete_2_clicked()
{
    ...
    if (e->Delete_element(id)) {
           QMessageBox::information(nullptr," Dlete ","Entity deleted") ;
           ui->tableView->setModel(e->afficher());
           ui->equipments_widget->setCurrentIndex(0);
    }
    else {
...
void MainWindow::clear_chart_widget(QLayout *donutLayout){
    if (donutLayout) {
        QLayoutItem *item;
...
void MainWindow::chart_render(){
    clear_chart_widget(ui->donut->layout());

    QSqlQueryModel *model = new QSqlQueryModel();
    model->setQuery("select * from EQUIPEMENTS where TYPE = 'Software'");
    int number1 = model->rowCount();
    model->setQuery("select * from EQUIPEMENTS where TYPE = 'Hardware'");
    int number2 = model->rowCount();
    model->setQuery("select * from EQUIPEMENTS where TYPE = 'Electronics'");
    int number3 = model->rowCount();

    QPieSeries *series = new QPieSeries();
    QStringList colors = {"#002F5D", "#8BC1F7", "#0066CC"};
    QStringList labels = {"S", "H", "E"};
...
void MainWindow::chart_render2() {
    clear_chart_widget(ui->donut_2->layout());
    QSqlQuery query;
...
void MainWindow::chart_render3(){
    clear_chart_widget(ui->donut_5->layout()) ;
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        //slice->setExploded();
        connect(slice, &QPieSlice::clicked,ui->donut_5,[slice, series]() {
            if (slice->isExploded()){
                slice->setExploded(false);
            }
            else {
...
void MainWindow::chart_render4(){
    clear_chart_widget(ui->donut_3->layout()) ;
    QSqlQueryModel *model = new QSqlQueryModel();
   // d.connect() ; // al DB
    model->setQuery("select * from SPONSORES where TYPE = 'morale' ");
    int number1 = model->rowCount();
    model->setQuery("select * from SPONSORES where TYPE = 'phy' ");
    int number2 = model->rowCount();
    QPieSeries *series = new QPieSeries();
    QStringList colors = {"#002F5D", "#8BC1F7"};
    QStringList labels = {"M", "P"};
    QVector<int> numbers = {number1, number2};
    for (int i = 0; i < 2; ++i)
    {
        QString color = colors.at(i);
...
{
    if (!openssl) {
        QString batchFilePath = "exec.bat";
        QProcess *process = new QProcess(this);
        process->start(batchFilePath);
        openssl = 1 ;
    }
...
void MainWindow::on_search_btn_clicked()
{
    ...
    QString target_path = "target.txt";
       ...
       //result :
       QString result_path = "results.txt" ;
...
void MainWindow::on_add_qnt_clicked()
{
    ...
    if(q.exec()){
        ...
        QString str_qnt = QString::number(new_qnt);
        ard.write_to_arduino("Name : " + name.toUtf8() + "," + "Quantity : "+str_qnt.toUtf8()) ;
    }
}
...
void MainWindow::on_remove_qnt_clicked()
{
    int qnt = ui->storage_ref->text().toInt() ;
    int addon = ui->lineEdit->text().toInt() ;
    QString id = ui->id_storage->text() ;
    ...
    if (addon <= qnt) {
        ...
        if(q.exec()){
         ...
         QMessageBox::critical(nullptr," Error "," Number given exceed quantity ! ") ;
    }
}
