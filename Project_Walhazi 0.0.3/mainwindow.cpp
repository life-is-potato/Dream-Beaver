...
void MainWindow::on_add_equip_clicked()
{
    int id= ui->ID->text().toInt();
            ...
            QString email= ui->type->text();
            int num= ui->qnt->text().toInt();
    ...
    // Check if the model contains any data before calling delete
        ...
        if (model->rowCount() > 0) {
            ...
            int id= ui->ID_2->text().toInt();
                    QString nom= ui->name_2->text();
                    QString email= ui->type_2->text();
                    int num = ui->qnt_2->text().toInt();
...
void MainWindow::on_tableView_activated(const QModelIndex &index)
{
    ...
    if (query.exec() && query.next()) {
        ...
        ui->qnt_2->setText(query.value(3).toString());
