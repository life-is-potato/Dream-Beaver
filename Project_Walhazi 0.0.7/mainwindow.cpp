...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindow), currentPage(0), sortOrderParticipants(Qt::AscendingOrder), sortOrderDate(Qt::AscendingOrder)
{
...
void MainWindow::on_pdf_formation_clicked()
{
    ...
    if (!saveFilePath.isEmpty()) {
        ...
        // Prepare SQL query to fetch formation data including the image column
        QSqlQuery query("SELECT ID, TYPE, INSTRUCTEUR, DD, DF, DESCRIPTION, NB_PARTICIPANTS, IMAGE FROM FORMATIONS", db);
        ...
        // Add table headers
        tableHtml += "<tr style=\"background-color: #ccc;\">";
        QStringList headers = { "ID", "Type", "Instructeur", "Date Début", "Date Fin", "Description", "Nombre de Participants", "Image" };
        ...
        while (query.next()) {
            ...
            for (int col = 0; col < query.record().count(); ++col) {
                if (query.record().fieldName(col) == "IMAGE") {
                    ...
                    pixmap.save(&buffer, "JPEG");  // Assuming the image format is JPEG, adjust if necessary
                    QString imageHtml = "<img src=\"data:image/jpeg;base64," + QString::fromLatin1(byteArray.toBase64()) + "\">";
                    tableHtml += "<td style=\"border: 1px solid black; padding: 8px;\">" + imageHtml + "</td>";
                } else {
...
void MainWindow::chart_render3(){
    ...
    int total = number1 + number2; // Calculate the total number of formations
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        double percentage = (static_cast<double>(numbers.at(i)) / total) * 100;
        slice->setLabel(QString("%1: %2%").arg(label).arg(percentage, 0, 'f', 1)); // Format label with percentage
...
void MainWindow::on_add_projet_clicked(){
    ...
    float budget=ui->budget_projet->text().toDouble();
...
void MainWindow::populateFormationWidgets(const QVector<FormationData> &searchResults) {
    ...
    // Create a new layout for widget_16 if it doesn't have one
    QGridLayout *widgetLayout = qobject_cast<QGridLayout*>(ui->widget_16->layout());
    if (!widgetLayout) {
        widgetLayout = new QGridLayout(ui->widget_16);
        ui->widget_16->setLayout(widgetLayout);
    }

    // Ensure widget_16 is visible
    ui->widget_16->setVisible(true);

    int totalFormations = searchResults.size();
    int maxFormations = 8; // Maximum number of cells in the grid layout
    int numRows = 2; // Number of rows in the grid layout

    // Populate formation widgets
    int formationIndex = currentPage * maxFormations;
    for (int row = 0; row < numRows; ++row) {
        for (int col = 0; col < 4; ++col) {
            if (formationIndex < totalFormations) {
                const FormationData &data = searchResults[formationIndex];
                addFormationWidget(data, widgetLayout, row, col);
                ++formationIndex;
            } else {
                // If there are fewer than 8 widgets, leave the remaining cells empty
                QWidget *emptyWidget = new QWidget;
                widgetLayout->addWidget(emptyWidget, row, col);
            }
        }
    }
}
...
void MainWindow::sortFormationWidgetsByDate() {
    std::sort(formationData.begin(), formationData.end(), [this](const FormationData &a, const FormationData &b) {
            return (sortOrderDate == Qt::AscendingOrder) ? a.dateDebut < b.dateDebut : a.dateDebut > b.dateDebut;
        });
        ...
        sortOrderDate = (sortOrderDate == Qt::AscendingOrder) ? Qt::DescendingOrder : Qt::AscendingOrder;
}


void MainWindow::clearFormationWidgets() {
    QLayout *layout = ui->widget_16->layout();
    if (layout) {
        QLayoutItem *item;
...
void MainWindow::sortFormationWidgetsByParticipants() {
    std::sort(formationData.begin(), formationData.end(), [this](const FormationData &a, const FormationData &b) {
            return (sortOrderParticipants == Qt::AscendingOrder) ? a.nbParticipants < b.nbParticipants : a.nbParticipants > b.nbParticipants;
        });
        ...
        // Toggle the sorting order for the next call
        sortOrderParticipants = (sortOrderParticipants == Qt::AscendingOrder) ? Qt::DescendingOrder : Qt::AscendingOrder;
}
...
void MainWindow::on_star_3_clicked()
{
    currentPage++;
